#!/bin/bash
# RantaiClaw Telegram Integration Test Suite
...
if [ -f "target/release/rantaiclaw" ]; then
    BINARY_SIZE=$(ls -lh target/release/rantaiclaw | awk '{print $5}')
    SIZE_BYTES=$(stat -f%z target/release/rantaiclaw 2>/dev/null || stat -c%s target/release/rantaiclaw)
...
CONFIG_PATH="$HOME/.rantaiclaw/config.toml"
if [ -f "$CONFIG_PATH" ]; then
    ...
    if grep -q "\[channels_config.telegram\]" "$CONFIG_PATH"; then
        ...
        warn "Telegram not configured - run 'rantaiclaw onboard' first"
    fi
else
    warn "No config file found - run 'rantaiclaw onboard' first"
fi
...
START_TIME=$(date +%s)
HEALTH_OUTPUT=$(timeout 10 target/release/rantaiclaw channel doctor 2>&1 || true)
END_TIME=$(date +%s)
