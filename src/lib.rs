...
Examples:
  ...
  rantaiclaw cron add-at 2025-12-31T23:59:00Z 'Happy New Year!'")]
    AddAt {
...
Examples:
  rantaiclaw cron add-every 60000 'Ping heartbeat'     # every minute
  rantaiclaw cron add-every 3600000 'Hourly report'    # every hour")]
    AddEvery {
...
Examples:
  ...
  rantaiclaw cron once 2h 'Follow up on deployment'
  rantaiclaw cron once 1d 'Daily check'")]
    Once {
        /// Delay duration
        delay: String,
        /// Command to run
...
Examples:
  rantaiclaw cron update <task-id> --expression '0 8 * * *'
  rantaiclaw cron update <task-id> --tz Europe/London --name 'Morning check'
  rantaiclaw cron update <task-id> --command 'Updated message'")]
    Update {
        /// Task ID
        id: String,
        /// New cron expression
...
Examples:
  ...
  rantaiclaw hardware introspect COM3")]
    Introspect {
...
Examples:
  ...
  rantaiclaw peripheral add esp32 /dev/ttyUSB0")]
    Add {
    ...
    /// Flash RantaiClaw firmware to Arduino (creates .ino, installs arduino-cli if needed, uploads)
    #[command(long_about = "\
Flash RantaiClaw firmware to an Arduino board.

Generates the .ino sketch, installs arduino-cli if it is not \
already available, compiles, and uploads the firmware.

Examples:
  rantaiclaw peripheral flash
  rantaiclaw peripheral flash --port /dev/cu.usbmodem12345
  rantaiclaw peripheral flash -p COM3")]
    Flash {
    ...
    /// Deploy RantaiClaw binary + config to Arduino Uno Q (cross-compiled aarch64)
    DeployUnoQ {
        /// Uno Q IP or user@host (e.g. 192.168.0.48 or arduino@192.168.0.48)
        #[arg(long)]
        host: String,
    },
    /// Flash RantaiClaw firmware to Nucleo-F401RE (builds + probe-rs run)
    FlashNucleo,
}
