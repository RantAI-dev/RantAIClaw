...
impl PrometheusObserver {
    pub fn new() -> Self {
        ...
        let agent_starts = IntCounterVec::new(
            prometheus::Opts::new("rantaiclaw_agent_starts_total", "Total agent invocations"),
            &["provider", "model"],
        )
        .expect("valid metric");

        let tool_calls = IntCounterVec::new(
            prometheus::Opts::new("rantaiclaw_tool_calls_total", "Total tool calls"),
            &["tool", "success"],
        )
        .expect("valid metric");
        ...
        let agent_duration = HistogramVec::new(
            HistogramOpts::new(
                "rantaiclaw_agent_duration_seconds",
        ...
        let tool_duration = HistogramVec::new(
            HistogramOpts::new(
                "rantaiclaw_tool_duration_seconds",
        ...
        let request_latency = Histogram::with_opts(
            HistogramOpts::new(
                "rantaiclaw_request_latency_seconds",
...
mod tests {
    ...
    fn encode_produces_prometheus_text_format() {
        ...
        assert!(output.contains("rantaiclaw_agent_starts_total"));
        assert!(output.contains("rantaiclaw_tool_calls_total"));
        assert!(output.contains("rantaiclaw_heartbeat_ticks_total"));
        assert!(output.contains("rantaiclaw_request_latency_seconds"));
    }
    ...
    fn counters_increment_correctly() {
        ...
        let output = obs.encode();
        assert!(output.contains("rantaiclaw_heartbeat_ticks_total 3"));
    }
    ...
    fn tool_calls_track_success_and_failure_separately() {
        ...
        let output = obs.encode();
        assert!(output.contains(r#"rantaiclaw_tool_calls_total{success="true",tool="shell"} 2"#));
        assert!(output.contains(r#"rantaiclaw_tool_calls_total{success="false",tool="shell"} 1"#));
    }
    ...
    fn errors_track_by_component() {
        ...
        let output = obs.encode();
        assert!(output.contains(r#"rantaiclaw_errors_total{component="provider"} 2"#));
