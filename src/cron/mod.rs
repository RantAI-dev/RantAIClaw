...
async fn run_job_report(config: &Config, id: &str) -> Result<String> {
    ...
    // A one-shot `cron run` from the CLI owns its process: its MCP servers are
    // spawned for this run and die with it.
    let (ok, output) =
        crate::cron::scheduler::run_job_manual(config, &security, &job, None, None).await;
