//! Flash RantaiClaw Arduino firmware via arduino-cli.
//!
...
/// RantaiClaw Arduino Uno base firmware (capabilities, gpio_read, gpio_write).
const FIRMWARE_INO: &str = include_str!("../../firmware/rantaiclaw-arduino/rantaiclaw-arduino.ino");

const FQBN: &str = "arduino:avr:uno";
const SKETCH_NAME: &str = "rantaiclaw-arduino";
...
/// Flash RantaiClaw firmware to Arduino at the given port.
pub fn flash_arduino_firmware(port: &str) -> Result<()> {
    ensure_arduino_cli()?;
    ensure_avr_core()?;

    let temp_dir = std::env::temp_dir().join(format!("rantaiclaw_flash_{}", uuid::Uuid::new_v4()));
    ...
    // Compile
    println!("Compiling RantaiClaw Arduino firmware...");
    ...
    println!("RantaiClaw firmware flashed successfully.");
