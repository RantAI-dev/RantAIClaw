//! Flash RantaiClaw Nucleo-F401RE firmware via probe-rs.
//!
...
/// Flash RantaiClaw Nucleo firmware. Builds from firmware/rantaiclaw-nucleo.
pub fn flash_nucleo_firmware() -> Result<()> {
    ...
    // CARGO_MANIFEST_DIR = repo root (rantaiclaw's Cargo.toml)
    let repo_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let firmware_dir = repo_root.join("firmware").join("rantaiclaw-nucleo");
    if !firmware_dir.join("Cargo.toml").exists() {
        anyhow::bail!(
            "Nucleo firmware not found at {}. Run from rantaiclaw repo root.",
            firmware_dir.display()
        );
    }

    println!("Building RantaiClaw Nucleo firmware...");
    ...
    println!("RantaiClaw Nucleo firmware flashed successfully.");
