...
pub struct TuiContext {
    ...
    /// Snapshot of `(name, configured, support, verification)` rows at TUI
    /// startup. Used by `/channels` and `/platforms` to render the table without
    /// needing live access to the on-disk config. Refreshed by `reload_config`.
    ///
    /// Both axes come from `CHANNEL_CATALOG`, not from a copy kept here. A
    /// hand-typed tier in the TUI is the same drift the console's duplicate
    /// catalog produced, and carrying only one of the two axes is how one word
    /// came to answer two questions in the first place.
    pub channels_summary: Vec<(
        String,
        bool,
