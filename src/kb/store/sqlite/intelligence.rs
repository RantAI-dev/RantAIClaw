...
impl IntelligenceStore for SqliteStore {
    ...
    async fn store_intelligence(
        ...
        tokio::task::spawn_blocking(move || -> KbResult<()> {
            ...
            // provisional id (Entity::id as generated by the caller) -> surviving
            // (first-seen, canonical_key-deduplicated) id. Same ON CONFLICT
            // semantics as `upsert_entity`: ties resolve to the first-seen row,
            // confidence refreshes to the max seen.
            let mut surviving_id: std::collections::HashMap<String, String> =
                std::collections::HashMap::new();
            {
                ...
                let mut select_id = tx.prepare("SELECT id FROM entity WHERE canonical_key = ?1")?;
                for e in &entities {
                    ...
                    let id: String =
                        select_id.query_row(params![e.canonical_key], |row| row.get(0))?;
                    surviving_id.insert(e.id.clone(), id);
                }
            }
            {
                ...
                for m in &mentions {
                    let entity_id = surviving_id.get(&m.entity_id).unwrap_or(&m.entity_id);
                    insert_mention.execute(params![
                        m.id,
                        entity_id,
                        m.document_id,
                        m.chunk_index,
            ...
            }
            {
                ...
                for r in &relations {
                    let source_id = surviving_id
                        .get(&r.source_entity_id)
                        .unwrap_or(&r.source_entity_id);
                    let target_id = surviving_id
                        .get(&r.target_entity_id)
                        .unwrap_or(&r.target_entity_id);
