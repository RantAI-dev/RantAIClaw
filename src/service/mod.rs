...
fn start_linux(init_system: InitSystem) -> Result<()> {
    match init_system {
        ...
        InitSystem::Openrc => {
            run_checked(Command::new("rc-service").args(["rantaiclaw", "start"]))?;
        }
...
fn status_linux(config: &Config, init_system: InitSystem) -> Result<()> {
    match init_system {
        ...
        InitSystem::Openrc => {
            let out = run_capture(Command::new("rc-service").args(["rantaiclaw", "status"]))
                .unwrap_or_else(|_| "unknown".into());
            println!("Service state: {}", out.trim());
            println!("Unit: /etc/init.d/rantaiclaw");
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    Ok(())
...
fn uninstall_linux(config: &Config, init_system: InitSystem) -> Result<()> {
    match init_system {
        ...
        InitSystem::Openrc => {
            let init_script = Path::new("/etc/init.d/rantaiclaw");
            if init_script.exists() {
                if let Err(err) =
                    run_checked(Command::new("rc-update").args(["del", "rantaiclaw", "default"]))
                {
                    eprintln!(
                        "⚠️  Warning: Could not remove rantaiclaw from OpenRC default runlevel: {err}"
                    );
                }
                fs::remove_file(init_script)
                    .with_context(|| format!("Failed to remove {}", init_script.display()))?;
            }
            println!("✅ Service uninstalled (/etc/init.d/rantaiclaw)");
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    Ok(())
...
/// Check if the rantaiclaw user exists and has expected properties.
...
fn check_rantaiclaw_user() -> Result<()> {
    ...
    let (del_cmd, add_cmd) = if is_alpine {
        (
            "deluser rantaiclaw && delgroup rantaiclaw",
        ...
        Ok(output) if output.status.success() => {
            ...
            if parts.len() >= 7 {
                ...
                if uid.parse::<u32>().unwrap_or(999) >= 1000 {
                    bail!(
                        "User 'rantaiclaw' exists but has unexpected UID {} (expected system UID < 1000).\n\
                ...
                if !shell.contains("nologin") && !shell.contains("false") {
                    bail!(
                        "User 'rantaiclaw' exists but has unexpected shell '{}'.\n\
                ...
                if home != "/var/lib/rantaiclaw" && home != "/nonexistent" {
                    eprintln!(
                        "⚠️  Warning: rantaiclaw user has home directory '{}' (expected /var/lib/rantaiclaw or /nonexistent)",
                        home
                    );
                }
...
fn ensure_rantaiclaw_user() -> Result<()> {
    ...
    if is_alpine {
        ...
        if !group_exists {
            let output = Command::new("addgroup")
                .args(["-S", "rantaiclaw"])
                .output()
                .context("Failed to create rantaiclaw group")?;

            if !output.status.success() {
                let stderr = String::from_utf8_lossy(&output.stderr);
                bail!("Failed to create rantaiclaw group: {}", stderr.trim());
            }
            println!("✅ Created system group: rantaiclaw");
        }
        ...
        let output = Command::new("adduser")
            ...
            .output()
            .context("Failed to create rantaiclaw user")?;

        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            bail!("Failed to create rantaiclaw user: {}", stderr.trim());
        }
    } else {
        let output = Command::new("useradd")
            .args(["-r", "-s", "/sbin/nologin", "rantaiclaw"])
            .output()
            .context("Failed to create rantaiclaw user")?;
    ...
    println!("✅ Created system user: rantaiclaw");
    Ok(())
}

/// Change ownership of a path to rantaiclaw:rantaiclaw
#[cfg(unix)]
fn chown_to_rantaiclaw(path: &Path) -> Result<()> {
    let output = Command::new("chown")
        .args(["rantaiclaw:rantaiclaw", &path.to_string_lossy()])
        .output()
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        bail!(
            "Failed to change ownership of {} to rantaiclaw:rantaiclaw: {}",
            path.display(),
            stderr.trim(),
        );
    }
...
#[cfg(not(unix))]
fn chown_to_rantaiclaw(_path: &Path) -> Result<()> {
    Ok(())
}
...
fn chown_recursive_to_rantaiclaw(path: &Path) -> Result<()> {
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        bail!(
            "Failed to recursively change ownership of {} to rantaiclaw:rantaiclaw: {}",
            path.display(),
            stderr.trim(),
        );
    }
...
#[cfg(not(unix))]
fn chown_recursive_to_rantaiclaw(_path: &Path) -> Result<()> {
    Ok(())
}
...
fn ensure_openrc_runtime_path_writable(path: &Path) -> Result<()> {
    ...
    if !output.status.success() {
        ...
        bail!(
            "OpenRC runtime user 'rantaiclaw' cannot write {} ({details}). \
             Re-run `sudo rantaiclaw service install` and ensure ownership is rantaiclaw:rantaiclaw.",
            path.display(),
        );
    }
    ...
    if path_str.contains("/home/") || path_str.contains(".cargo/bin") {
        eprintln!(
            "⚠️  Warning: Binary path '{}' appears to be in a user home directory.\n\
             ...
             sudo cp '{}' /usr/local/bin/rantaiclaw",
            exe_path.display(),
            exe_path.display()
        );
    }
...
command_user="rantaiclaw:rantaiclaw"
pidfile="/run/${{RC_SVCNAME}}.pid"
umask 027
output_log="/var/log/rantaiclaw/access.log"
error_log="/var/log/rantaiclaw/error.log"
...
fn resolve_openrc_executable() -> Result<PathBuf> {
    let preferred = Path::new("/usr/local/bin/rantaiclaw");
    if preferred.exists() {
...
fn install_linux_openrc(config: &Config) -> Result<()> {
    if !is_root() {
        bail!(
            "OpenRC service installation requires root privileges.\n\
             Please run with sudo: sudo rantaiclaw service install"
        );
    }

    ensure_rantaiclaw_user()?;

    let exe = resolve_openrc_executable()?;
    warn_if_binary_in_home(&exe);

    let config_dir = Path::new("/etc/rantaiclaw");
    let workspace_dir = config_dir.join("workspace");
    let log_dir = Path::new("/var/log/rantaiclaw");
    ...
    if !workspace_dir.exists() {
        ...
        chown_to_rantaiclaw(&workspace_dir)?;
        println!(
            "✅ Created directory: {} (owned by rantaiclaw:rantaiclaw)",
    ...
    chown_recursive_to_rantaiclaw(config_dir)?;
    ...
    run_checked(Command::new("rc-update").args(["add", "rantaiclaw", "default"]))?;
    println!("✅ Installed OpenRC service: /etc/init.d/rantaiclaw");
    println!("   Config path: /etc/rantaiclaw/config.toml");
    println!("   Start with: sudo rantaiclaw service start");
    let _ = config;
    Ok(())
}
...
fn install_windows(config: &Config) -> Result<()> {
    ...
    let wrapper = logs_dir.join("rantaiclaw-daemon.cmd");
...
mod tests {
    ...
    fn windows_task_name_is_constant() {
        assert_eq!(windows_task_name(), "RantaiClaw Daemon");
    }
    ...
    fn generate_openrc_script_contains_required_directives() {
        ...
        assert!(!script.contains("env RANTAICLAW_WORKSPACE"));
        assert!(script.contains("command_background=\"yes\""));
        assert!(script.contains("command_user=\"rantaiclaw:rantaiclaw\""));
        assert!(script.contains("pidfile=\"/run/${RC_SVCNAME}.pid\""));
        assert!(script.contains("umask 027"));
        assert!(script.contains("output_log=\"/var/log/rantaiclaw/access.log\""));
        assert!(script.contains("error_log=\"/var/log/rantaiclaw/error.log\""));
    ...
    fn warn_if_binary_in_home_detects_home_path() {
        ...
        let home_path = PathBuf::from("/home/user/.cargo/bin/rantaiclaw");
        assert!(home_path.to_string_lossy().contains("/home/"));
        assert!(home_path.to_string_lossy().contains(".cargo/bin"));

        let cargo_path = PathBuf::from("/home/user/.cargo/bin/rantaiclaw");
        assert!(cargo_path.to_string_lossy().contains(".cargo/bin"));

        let system_path = PathBuf::from("/usr/local/bin/rantaiclaw");
    ...
    fn openrc_writability_probe_prefers_runuser_when_available() {
        let (program, args) =
            build_openrc_writability_probe_command(Path::new("/etc/rantaiclaw"), true);
        ...
        assert_eq!(
            ...
            vec![
                ...
                "sh".to_string(),
                "-c".to_string(),
                "test -w '/etc/rantaiclaw'".to_string()
            ]
        );
    }
    ...
    fn openrc_writability_probe_falls_back_to_su() {
        let (program, args) =
            build_openrc_writability_probe_command(Path::new("/etc/rantaiclaw/workspace"), false);
        ...
        assert_eq!(
            ...
            vec![
                ...
                "/bin/sh".to_string(),
                "-c".to_string(),
                "test -w '/etc/rantaiclaw/workspace'".to_string(),
                "rantaiclaw".to_string()
            ]
        );
    }
