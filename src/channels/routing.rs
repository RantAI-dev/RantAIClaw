...
pub(crate) fn channel_thread_replies(cc: &crate::config::ChannelsConfig) -> HashMap<String, bool> {
    ...
    for (name, _, _, _) in CHANNEL_CATALOG {
...
pub(crate) fn channel_allowlists(
    ...
    // Cloud (`"whatsapp"`) and Web (`"whatsapp_web"`) each need their
    // own entry; inbound messages arrive keyed by the runtime name.
    put("whatsapp", cc.whatsapp.as_ref().map(|c| &c.allowed_numbers));
    put(
        "whatsapp_web",
        cc.whatsapp_web.as_ref().map(|c| &c.allowed_numbers),
    );
