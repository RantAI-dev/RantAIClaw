...
SYSTEM_PROMPT = """You are RantaiClaw, an AI assistant with tool access. Use tools to accomplish tasks.
...
class RantaiclawAgent:
    ...
    def invoke(self, input: dict[str, Any], config: Optional[dict] = None) -> dict:
        ...
        raise RuntimeError(
            "RantaiclawAgent.invoke() cannot be called inside an active event loop. "
            "Use 'await RantaiclawAgent.ainvoke(...)' instead."
        )
...
def create_agent(
    ...
    """
    Create a RantaiClaw agent with LangGraph-based tool calling.
    ...
    Returns:
        Configured RantaiclawAgent instance
