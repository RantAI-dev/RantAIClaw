...
|---|---|---|---|
| 2026-02-19 | _Nghiêm trọng_ | Chúng tôi **không có liên kết** với `openagen/rantaiclaw` hoặc `rantaiclaw.org`. Tên miền `rantaiclaw.org` hiện đang trỏ đến fork `openagen/rantaiclaw`, và tên miền/repository đó đang mạo danh website/dự án chính thức của chúng tôi. | Không tin tưởng thông tin, binary, gây quỹ, hay thông báo từ các nguồn đó. Chỉ sử dụng repository này và các tài khoản mạng xã hội đã được xác minh của chúng tôi. |
| 2026-02-19 | _Quan trọng_ | Chúng tôi **chưa** ra mắt website chính thức, và chúng tôi đang ghi nhận các nỗ lực mạo danh. **Không** tham gia bất kỳ hoạt động đầu tư hoặc gây quỹ nào tuyên bố mang tên RantaiClaw. | Sử dụng repository này làm nguồn thông tin duy nhất đáng tin cậy. Theo dõi [X (@rantaiclawlabs)](https://x.com/rantaiclawlabs?s=21), [Reddit (r/rantaiclawlabs)](https://www.reddit.com/r/rantaiclawlabs/), [Telegram (@rantaiclawlabs)](https://t.me/rantaiclawlabs), [Telegram CN (@rantaiclawlabs_cn)](https://t.me/rantaiclawlabs_cn), [Telegram RU (@rantaiclawlabs_ru)](https://t.me/rantaiclawlabs_ru), và [Xiaohongshu](https://www.xiaohongshu.com/user/profile/67cbfc43000000000d008307?xsec_token=AB73VnYnGNx5y36EtnnZfGmAmS-6Wzv8WMuGpfwfkg6Yc%3D&xsec_source=pc_search) để nhận cập nhật chính thức. |
...
### Vì sao các team chọn RantaiClaw
...
## So sánh hiệu suất (RantaiClaw vs OpenClaw, có thể tái tạo)
...
> Ghi chú: Kết quả RantaiClaw được đo trên release build sử dụng `/usr/bin/time -l`. OpenClaw yêu cầu runtime Node.js (thường thêm ~390MB bộ nhớ overhead), còn NanoBot yêu cầu runtime Python. PicoClaw và RantaiClaw là các static binary. Số RAM ở trên là bộ nhớ runtime; yêu cầu biên dịch lúc build-time sẽ cao hơn.
...
- `rantaiclaw --help`: khoảng `0.02s`, bộ nhớ đỉnh ~`3.9MB`
- `rantaiclaw status`: khoảng `0.01s`, bộ nhớ đỉnh ~`4.1MB`
...
Hoặc bỏ qua các bước trên, cài hết mọi thứ (system deps, Rust, RantaiClaw) chỉ bằng một lệnh:

```bash
curl -LsSf https://raw.githubusercontent.com/rantaiclaw-labs/rantaiclaw/main/scripts/install.sh | bash
```
...
<https://github.com/rantaiclaw-labs/rantaiclaw/releases/latest>

Ví dụ (ARM64 Linux):

```bash
curl -fsSLO https://github.com/rantaiclaw-labs/rantaiclaw/releases/latest/download/rantaiclaw-aarch64-unknown-linux-gnu.tar.gz
...
rantaiclaw gateway                # mặc định: 127.0.0.1:3000
rantaiclaw gateway --port 0       # cổng ngẫu nhiên (tăng cường bảo mật)
...
RantaiClaw hỗ trợ profile xác thực theo gói đăng ký (đa tài khoản, mã hóa khi lưu).

- File lưu trữ: `~/.rantaiclaw/auth-profiles.json`
- Khóa mã hóa: `~/.rantaiclaw/.secret_key`
...
|-----------|-------|------------|--------|
| **Mô hình AI** | `Provider` | Danh mục provider qua `rantaiclaw providers` (hiện có 28 built-in + alias, cộng endpoint tùy chỉnh) | `custom:https://your-api.com` (tương thích OpenAI) hoặc `anthropic-custom:https://your-api.com` |
...
Khi cấu hình `runtime.kind` không được hỗ trợ, RantaiClaw sẽ thoát với thông báo lỗi rõ ràng thay vì âm thầm fallback về native.
...
# Khi provider = "postgres", RantaiClaw dùng PostgreSQL để lưu memory.
...
# RANTAICLAW_LUCID_CMD=/usr/local/bin/lucid            # mặc định: lucid
# RANTAICLAW_LUCID_BUDGET=200                          # mặc định: 200
# RANTAICLAW_LUCID_LOCAL_HIT_THRESHOLD=3               # số lần hit cục bộ để bỏ qua external recall
# RANTAICLAW_LUCID_RECALL_TIMEOUT_MS=120               # giới hạn thời gian cho lucid context recall
# RANTAICLAW_LUCID_STORE_TIMEOUT_MS=800                # timeout đồng bộ async cho lucid store
# RANTAICLAW_LUCID_FAILURE_COOLDOWN_MS=15000           # thời gian nghỉ sau lỗi lucid, tránh thử lại liên tục
```

## Bảo mật

RantaiClaw thực thi bảo mật ở **mọi lớp** — không chỉ sandbox. Đáp ứng tất cả các hạng mục trong danh sách kiểm tra bảo mật của cộng đồng.
...
> **Tự chạy nmap:** `nmap -p 1-65535 <your-host>` — RantaiClaw chỉ bind vào localhost, nên không có gì bị lộ ra ngoài trừ khi bạn cấu hình tunnel tường minh.
...
Với các phản hồi không phải văn bản, RantaiClaw có thể gửi file đính kèm Telegram khi assistant bao gồm các marker:
...
RantaiClaw hỗ trợ hai backend WhatsApp:
...
3. **Khởi động channel/daemon và liên kết thiết bị:**
   - Chạy `rantaiclaw channel start` (hoặc `rantaiclaw daemon`).
...
6. **Kiểm tra:** Gửi tin nhắn đến số WhatsApp Business của bạn — RantaiClaw sẽ phản hồi qua LLM.

## Cấu hình

Config: `~/.rantaiclaw/config.toml` (được tạo bởi `onboard`)

Khi `rantaiclaw channel start` đang chạy, các thay đổi với `default_provider`,
...
RantaiClaw dùng một khóa provider (`ollama`) cho cả triển khai Ollama cục bộ và từ xa:
...
## Gói Python đi kèm (`rantaiclaw-tools`)

Với các LLM provider có tool calling native không ổn định (ví dụ: GLM-5/Zhipu), RantaiClaw đi kèm gói Python dùng **LangGraph để gọi tool** nhằm đảm bảo tính nhất quán:
...
RantaiClaw hỗ trợ persona AI **không phụ thuộc nền tảng** qua hai định dạng:
...
[AIEOS](https://aieos.org) là framework chuẩn hóa cho định danh AI di động. RantaiClaw hỗ trợ payload AIEOS v1.1 JSON, cho phép bạn:
...
RantaiClaw chấp nhận cả payload AIEOS đầy đủ lẫn dạng rút gọn, rồi chuẩn hóa về một định dạng system prompt thống nhất.
...
Bạn cũng có thể ghi đè lúc runtime với `RANTAICLAW_OPEN_SKILLS_ENABLED` và `RANTAICLAW_OPEN_SKILLS_DIR`.
...
RantaiClaw được cấu hình để dùng `rustls` cho các dependencies HTTP/TLS; `--locked` giữ cho dependency graph nhất quán trên các môi trường mới.
...
Nếu RantaiClaw giúp ích cho công việc của bạn và bạn muốn hỗ trợ phát triển liên tục, bạn có thể quyên góp tại đây:
...
**Đây là repository RantaiClaw chính thức duy nhất:**
> https://github.com/rantaiclaw-labs/rantaiclaw

Bất kỳ repository, tổ chức, tên miền hay gói nào khác tuyên bố là "RantaiClaw" hoặc ngụ ý liên kết với RantaiClaw Labs đều là **không được ủy quyền và không liên kết với dự án này**. Các fork không được ủy quyền đã biết sẽ được liệt kê trong [TRADEMARK.md](TRADEMARK.md).

Nếu bạn phát hiện hành vi mạo danh hoặc lạm dụng nhãn hiệu, vui lòng [mở một issue](https://github.com/rantaiclaw-labs/rantaiclaw/issues).

---

## Giấy phép

RantaiClaw được cấp phép kép để tối đa hóa tính mở và bảo vệ người đóng góp:
...
Tên **RantaiClaw** và logo là nhãn hiệu của RantaiClaw Labs. Giấy phép này không cấp phép sử dụng chúng để ngụ ý chứng thực hoặc liên kết. Xem [TRADEMARK.md](TRADEMARK.md) để biết các sử dụng được phép và bị cấm.
...
- `Skill` mới → `~/.rantaiclaw/workspace/skills/<name>/`

---

**RantaiClaw** — Không tốn thêm tài nguyên. Không đánh đổi. Triển khai ở đâu cũng được. Thay thế gì cũng được. 🦀
...
<p align="center">
  <a href="https://www.star-history.com/#rantaiclaw-labs/rantaiclaw&type=date&legend=top-left">
    <picture>
     ...
     <img alt="Star History Chart" src="https://api.star-history.com/svg?repos=rantaiclaw-labs/rantaiclaw&type=date&legend=top-left" />
    </picture>
  </a>
</p>
