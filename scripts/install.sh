...
INSTALLER_LOCAL="$(cd "$SCRIPT_DIR/.." >/dev/null 2>&1 && pwd || pwd)/rantaiclaw_install.sh"
BOOTSTRAP_LOCAL="$SCRIPT_DIR/bootstrap.sh"
REPO_URL="${RANTAICLAW_REPO_URL:-https://github.com/RantAI-dev/RantAIClaw.git}"

echo "[deprecated] scripts/install.sh -> ./rantaiclaw_install.sh" >&2
...
TEMP_DIR="$(mktemp -d -t rantaiclaw-install-XXXXXX)"
cleanup() {
  rm -rf "$TEMP_DIR"
}
trap cleanup EXIT
...
if [[ -x "$TEMP_DIR/rantaiclaw_install.sh" ]]; then
  exec "$TEMP_DIR/rantaiclaw_install.sh" "$@"
fi
...
echo "error: rantaiclaw_install.sh/bootstrap.sh was not found in the fetched revision." >&2
echo "Run the local bootstrap directly when possible:" >&2
echo "  ./rantaiclaw_install.sh --help" >&2
exit 1
