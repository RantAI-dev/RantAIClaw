# RantaiClaw Security Improvement Roadmap
...
RantaiClaw already has **excellent application-layer security**:
...
## Comparison: RantaiClaw vs PicoClaw vs Production Grade

| Feature | PicoClaw | RantaiClaw Now | RantaiClaw + Roadmap | Production Target |
|---------|----------|--------------|-------------------|-------------------|
...
| Security self-test (`rantaiclaw audit --check`) | 1 day | Low |
...
[security.audit]
enabled = true
log_path = "~/.config/rantaiclaw/audit.log"
sign_events = true
max_size_mb = 100
...
rantaiclaw security --check
# → ✓ Sandbox: Firejail active
# → ✓ Audit logging enabled (42 events today)
# → → Resource limits: 512MB mem, 50% CPU

# Audit log queries
rantaiclaw audit --user @alice --since 24h
rantaiclaw audit --risk high --violations-only
rantaiclaw audit --verify-signatures

# Sandbox test
rantaiclaw sandbox --test
# → Testing isolation...
...
**RantaiClaw is already more secure than PicoClaw** with:
...
**By implementing this roadmap**, RantaiClaw becomes:
...
**Value**: Transforms RantaiClaw from "safe for testing" to "safe for production"
