...
### Không tìm thấy lệnh `rantaiclaw` sau cài đặt

Biểu hiện:

- Cài đặt thành công nhưng shell không tìm thấy `rantaiclaw`
...
Xác minh `~/.rantaiclaw/config.toml`:
...
- Dừng các tiến trình `rantaiclaw daemon` / `rantaiclaw channel start` thừa
