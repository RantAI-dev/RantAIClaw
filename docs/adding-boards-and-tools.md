# Adding Boards and Tools — RantaiClaw Hardware Guide

This guide explains how to add new hardware boards and custom tools to RantaiClaw.

## Quick Start: Add a Board via CLI

```bash
# Add a board (updates ~/.rantaiclaw/config.toml)
rantaiclaw peripheral add nucleo-f401re /dev/ttyACM0
rantaiclaw peripheral add arduino-uno /dev/cu.usbmodem12345
rantaiclaw peripheral add rpi-gpio native   # for Raspberry Pi GPIO (Linux)
...
With the `rag-pdf` feature, RantaiClaw can index PDF files:
...
2. **Add to config** — `rantaiclaw peripheral add my-board /dev/ttyUSB0`
...
| Command | Description |
|---------|-------------|
| `rantaiclaw peripheral list` | List configured boards |
| `rantaiclaw peripheral add <board> <path>` | Add board (writes config) |
| `rantaiclaw peripheral flash` | Flash Arduino firmware |
| `rantaiclaw peripheral flash-nucleo` | Flash Nucleo firmware |
| `rantaiclaw hardware discover` | List USB devices |
| `rantaiclaw hardware info` | Chip info via probe-rs |
