# Triển khai mạng — RantaiClaw trên Raspberry Pi và mạng nội bộ

Tài liệu này hướng dẫn triển khai RantaiClaw trên Raspberry Pi hoặc host khác trong mạng nội bộ, với các channel Telegram và webhook tùy chọn.
...
|------|----------------------|----------|
| **Telegram polling** | Không | RantaiClaw poll Telegram API; hoạt động từ bất kỳ đâu |
| **Matrix sync (kể cả E2EE)** | Không | RantaiClaw sync qua Matrix client API; không cần webhook đến |
...
**Lưu ý:** Telegram, Discord và Slack dùng **long-polling** — RantaiClaw thực hiện các request ra ngoài. Không cần port forwarding hoặc public IP.

---

## 2. RantaiClaw trên Raspberry Pi
...
Chỉnh sửa `~/.rantaiclaw/config.toml`:
...
- Channel Telegram hoạt động: RantaiClaw poll Telegram API (outbound)
...
2. Khởi động tunnel:
   ...
   ```
   Hoặc dùng `rantaiclaw tunnel` (xem tài liệu tunnel).

3. RantaiClaw sẽ từ chối `0.0.0.0` trừ khi `allow_public_bind = true` hoặc có tunnel đang hoạt động.
...
- RantaiClaw gọi `https://api.telegram.org/bot{token}/getUpdates`
...
Chạy `rantaiclaw daemon` — channel Telegram khởi động tự động.
...
- Chỉ chạy một instance runtime cho cùng token (khuyến nghị: service `rantaiclaw daemon`).
...
- [ ] Chạy `rantaiclaw daemon --host 127.0.0.1 --port 3000` (Telegram hoạt động không cần 0.0.0.0)
