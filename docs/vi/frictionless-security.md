...
[INFO] ✓ Audit logging enabled (~/.config/rantaiclaw/audit.log)
...
$ rantaiclaw security --status
Security Status:
  ✓ Sandbox: Landlock (Linux kernel 6.2)
  ✓ Memory monitoring: 512MB limit
  ✓ Audit logging: ~/.config/rantaiclaw/audit.log
  → 47 events logged today

# Tắt sandbox tường minh (ghi vào config)
$ rantaiclaw config set security.sandbox.enabled false

# Bật backend cụ thể
$ rantaiclaw config set security.sandbox.backend firejail

# Điều chỉnh giới hạn
$ rantaiclaw config set security.resources.max_memory_mb 2048
```
...
✅ **Hiển thị trạng thái** — `rantaiclaw security --status`
