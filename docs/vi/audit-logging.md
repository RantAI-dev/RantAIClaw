...
## Vấn đề
RantaiClaw ghi log các hành động nhưng thiếu audit trail chống giả mạo cho:
...
# Chống giả mạo
sign_events = true
signing_key_path = "~/.config/rantaiclaw/audit.key"
...
rantaiclaw audit --user @alice

# Hiển thị tất cả lệnh rủi ro cao
rantaiclaw audit --risk high

# Hiển thị vi phạm trong 24 giờ qua
rantaiclaw audit --since 24h --violations-only

# Xuất sang JSON để phân tích
rantaiclaw audit --format json --output audit.json
