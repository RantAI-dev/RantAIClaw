...
Hướng dẫn này giải thích cách sử dụng gói Python `rantaiclaw-tools` để gọi tool nhất quán với bất kỳ LLM provider nào tương thích OpenAI.

## Bối cảnh

Một số LLM provider, đặc biệt là các model Trung Quốc như GLM-5 (Zhipu AI), có hành vi gọi tool không nhất quán khi dùng phương thức text-based tool invocation. Core Rust của RantaiClaw sử dụng structured tool calling theo định dạng OpenAI API, nhưng một số model phản hồi tốt hơn với cách tiếp cận khác.
...
```python
import asyncio
from rantaiclaw_tools import create_agent, shell, file_read, file_write
...
```python
from rantaiclaw_tools import tool, create_agent
...
```python
import os
from rantaiclaw_tools.integrations import DiscordBot
...
# Single message
rantaiclaw-tools "What is the current date?"

# Interactive mode
rantaiclaw-tools -i
```

## So sánh với Rust RantaiClaw

| Khía cạnh | Rust RantaiClaw | rantaiclaw-tools |
|--------|---------------|-----------------|
