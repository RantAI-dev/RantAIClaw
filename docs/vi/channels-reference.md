...
Tài liệu này là nguồn tham khảo chính thức về cấu hình channel trong RantaiClaw.
...
6. **Trạng thái runtime cũ**: config đã thay đổi nhưng `rantaiclaw daemon` chưa được khởi động lại.

---

## 1. Namespace cấu hình

Tất cả cài đặt channel nằm trong `channels_config` trong `~/.rantaiclaw/config.toml`.
...
Khi chạy `rantaiclaw channel start` (hoặc chế độ daemon), Telegram và Discord hỗ trợ chuyển đổi runtime theo phạm vi người gửi:
...
- Xem trước bộ nhớ cache model từ `rantaiclaw models refresh --provider <ID>`.
- Đây là lệnh chat runtime, không phải lệnh con CLI.

## Giao thức marker hình ảnh đầu vào

RantaiClaw hỗ trợ đầu vào multimodal qua các marker nội tuyến trong tin nhắn:
...
Nếu `[channels_config.matrix]` có mặt nhưng binary được build mà không có `channel-matrix`, các lệnh `rantaiclaw channel list`, `rantaiclaw channel doctor`, và `rantaiclaw channel start` sẽ ghi log rằng Matrix bị bỏ qua có chủ ý trong bản build này.
...
access_token = "syt_..."
user_id = "@rantaiclaw:matrix.example.com"   # tùy chọn, khuyến nghị cho E2EE
...
5. Khởi động lại `rantaiclaw daemon` sau khi thay đổi config.
