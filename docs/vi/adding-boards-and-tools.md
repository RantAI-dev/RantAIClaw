# Thêm Board và Tool — Hướng dẫn phần cứng RantaiClaw

Hướng dẫn này giải thích cách thêm board phần cứng mới và tool tùy chỉnh vào RantaiClaw.

## Bắt đầu nhanh: Thêm board qua CLI

```bash
# Thêm board (cập nhật ~/.rantaiclaw/config.toml)
rantaiclaw peripheral add nucleo-f401re /dev/ttyACM0
rantaiclaw peripheral add arduino-uno /dev/cu.usbmodem12345
rantaiclaw peripheral add rpi-gpio native   # cho Raspberry Pi GPIO (Linux)
...
Với feature `rag-pdf`, RantaiClaw có thể lập chỉ mục file PDF:
...
2. **Thêm vào config** — `rantaiclaw peripheral add my-board /dev/ttyUSB0`
...
| Lệnh | Mô tả |
|------|-------|
| `rantaiclaw peripheral list` | Liệt kê các board đã cấu hình |
| `rantaiclaw peripheral add <board> <path>` | Thêm board (ghi vào config) |
| `rantaiclaw peripheral flash` | Nạp firmware Arduino |
| `rantaiclaw peripheral flash-nucleo` | Nạp firmware Nucleo |
| `rantaiclaw hardware discover` | Liệt kê thiết bị USB |
| `rantaiclaw hardware info` | Thông tin chip qua probe-rs |
