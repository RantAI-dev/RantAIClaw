...
This guide covers native Nextcloud Talk integration for RantaiClaw.
...
Add this section in `~/.rantaiclaw/config.toml`:
...
rantaiclaw daemon
# or
rantaiclaw gateway --host 127.0.0.1 --port 3000
```
...
When `webhook_secret` is configured, RantaiClaw verifies:
...
- RantaiClaw ignores bot-originated webhook events (`actorType = bots`).
- RantaiClaw ignores non-message/system events.
...
3. Confirm RantaiClaw receives and replies in the same room.
