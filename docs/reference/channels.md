...
- **Socket Mode** holds one outbound WebSocket carrying events for every
  ...
  public (`channel_type` `channel`) and private (`channel_type` `private`)
  channels to that id; a direct message (`channel_type` `im`) always reaches the
  bot, since its `D…` id can never equal a configured `C…` or `G…` id. No public
  inbound port either way.
