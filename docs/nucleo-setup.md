# RantaiClaw on Nucleo-F401RE — Step-by-Step Guide

Run RantaiClaw on your Mac or Linux host. Connect a Nucleo-F401RE via USB. Control GPIO (LED, pins) via Telegram or CLI.

---

## Get Board Info via Telegram (No Firmware Needed)

RantaiClaw can read chip info from the Nucleo over USB **without flashing any firmware**. Message your Telegram bot:
...
RantaiClaw includes everything for Nucleo-F401RE:
...
| Flash command | `rantaiclaw peripheral flash-nucleo` | Builds firmware, flashes via probe-rs |
...
### 1.2 Flash via RantaiClaw

From the rantaiclaw repo root:

```bash
rantaiclaw peripheral flash-nucleo
```

This builds `firmware/rantaiclaw-nucleo` and runs `probe-rs run --chip STM32F401RETx`. The firmware runs immediately after flashing.
...
| 5 | `rantaiclaw daemon` or `rantaiclaw agent -m "Turn on LED"` |
...
- **GPIO commands ignored** — Check `path` in config matches your serial port. Run `rantaiclaw peripheral list` to verify.
