...
RantaiClaw supports native integration with Mattermost via its REST API v4. This integration is ideal for self-hosted, private, or air-gapped environments where sovereign communication is a requirement.
...
2.  **Bot Account**:
    ...
    - Set a username (e.g., `rantaiclaw-bot`).
...
| `mention_only` | (Optional) When `true`, only messages that explicitly mention the bot username (for example `@rantaiclaw-bot`) are processed. Default: `false`. |

## Threaded Conversations

RantaiClaw supports Mattermost threads in both modes:
- If a user sends a message in an existing thread, RantaiClaw always replies within that same thread.
...
When `mention_only = true`, RantaiClaw applies an extra filter after `allowed_users` authorization:
