# CLAUDE.md — RantaiClaw Agent Engineering Protocol
...
RantaiClaw is a Rust-first autonomous agent runtime optimized for:
...
- If identity-like naming is required in tests/examples, use RantaiClaw-native labels only (`RantaiClawAgent`, `rantaiclaw_user`, `rantaiclaw_node`).
...
- If identity-like context is unavoidable, use RantaiClaw-scoped roles/labels only (for example: `RantaiClawAgent`, `RantaiClawOperator`, `rantaiclaw_user`) and avoid real-world personas.
- Recommended identity-safe naming palette (use when identity-like context is required):
    - actor labels: `RantaiClawAgent`, `RantaiClawOperator`, `RantaiClawMaintainer`, `rantaiclaw_user`
    - service/runtime labels: `rantaiclaw_bot`, `rantaiclaw_service`, `rantaiclaw_runtime`, `rantaiclaw_node`
    - environment labels: `rantaiclaw_project`, `rantaiclaw_workspace`, `rantaiclaw_channel`
