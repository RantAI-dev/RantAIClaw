//! Performance benchmarks for RantaiClaw hot paths.
//!
//! Benchmarks cover:
...
//! Run: `cargo bench`
//!
//! Ref: https://github.com/rantaiclaw-labs/rantaiclaw/issues/618 (item 7)
...
use rantaiclaw::agent::dispatcher::{NativeToolDispatcher, ToolDispatcher, XmlToolDispatcher};
use rantaiclaw::config::MemoryConfig;
use rantaiclaw::memory;
use rantaiclaw::memory::{Memory, MemoryCategory};
...
<tool_call>
{"name": "search", "arguments": {"query": "rantaiclaw architecture"}}
</tool_call>
    ...
    let response = ChatResponse {
        ...
        tool_calls: vec![
            ToolCall {
                id: "tc1".into(),
                name: "search".into(),
                arguments: r#"{"query": "rantaiclaw"}"#.into(),
            },
            ToolCall {
                id: "tc2".into(),
                name: "read_file".into(),
...
fn bench_memory_operations(c: &mut Criterion) {
    ...
    rt.block_on(async {
        for i in 0..100 {
            mem.store(
                &format!("key_{i}"),
                &format!("Content entry number {i} about rantaiclaw agent runtime"),
                MemoryCategory::Core,
                None,
            )
            .await
