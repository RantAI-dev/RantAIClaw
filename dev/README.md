# RantaiClaw Development Environment

A fully containerized development sandbox for RantaiClaw agents. This environment allows you to develop, test, and debug the agent in isolation without modifying your host system.
...
### 2. Enter Agent Container (`rantaiclaw-dev`)

```bash
./dev/cli.sh agent
```

Use this to run `rantaiclaw` CLI commands manually, debug the binary, or check logs internally.

- **Path**: `/rantaiclaw-data`
- **User**: `nobody` (65534)
...
- **Agent**: `/rantaiclaw-data/workspace`
- **Sandbox**: `/home/developer/workspace`

Files created by the agent are visible to the sandbox user, and vice versa.

The agent configuration lives in `target/.rantaiclaw` (mounted to `/rantaiclaw-data/.rantaiclaw`), so settings persist across container rebuilds.
...
**Note:** This removes `target/.rantaiclaw` (config/DB) but leaves the `playground/` directory intact. To fully wipe everything, manually delete `playground/`.
...
- Rust compilation, tests, and audit/deny tools run in `rantaiclaw-local-ci` container.
...
- The root `Dockerfile` also caches Rust `target/` (`id=rantaiclaw-target`) to speed repeat local image builds.
