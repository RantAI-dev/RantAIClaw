...
This guide covers testing the Telegram channel integration for RantaiClaw.
...
6. **Error logging**
    ...
    ```bash
    RUST_LOG=debug rantaiclaw channel start
    ```
...
6. **Health check timeout**
    ...
    ```bash
    time rantaiclaw channel doctor
    ```
...
Solution: Check user allowlist
  ...
  4. Run: rantaiclaw onboard --channels-only
```
...
# 3. Monitor logs
RUST_LOG=info rantaiclaw daemon
...
| ---------------------- | ---------- | -------------------------------- |
| Health check time      | <5s        | `time rantaiclaw channel doctor`   |
| First response time    | <3s        | Time from sending to receiving   |
| Message split overhead | <50ms      | Check logs for timing            |
| Memory usage           | <10MB      | `ps aux \| grep rantaiclaw`        |
| Binary size            | ~3-4MB     | `ls -lh target/release/rantaiclaw` |
...
```bash
# Maximum logging
RUST_LOG=trace rantaiclaw channel start
...
# Validate config
cat ~/.rantaiclaw/config.toml | grep -A 3 "\[channels_config.telegram\]"
```
...
- [RantaiClaw Main README](README.md)
- [Contributing Guide](CONTRIBUTING.md)
- [Issue Tracker](https://github.com/theonlyhennygod/rantaiclaw/issues)
