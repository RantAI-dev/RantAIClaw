# RantaiClaw Robot Kit

A complete toolkit for building AI-powered robots with RantaiClaw. Designed for Raspberry Pi deployment with offline Ollama inference.
...
### 1. Build RantaiClaw with robot tools
...
# Copy config
mkdir -p ~/.rantaiclaw
cp crates/robot-kit/robot.toml ~/.rantaiclaw/
cp crates/robot-kit/SOUL.md ~/.rantaiclaw/workspace/
...
# Test in mock mode
./target/release/rantaiclaw agent -m "Say hello and show a happy face"

# Test with real hardware
# (after configuring robot.toml)
./target/release/rantaiclaw agent -m "Move forward 1 meter"
```
...
```rust
use rantaiclaw_robot_kit::{create_tools, RobotConfig};
...
If you want runtime registration in `rantaiclaw`, add a thin adapter that maps this
...
mkdir rantaiclaw-robot-kit
cp -r target/release/rantaiclaw rantaiclaw-robot-kit/
cp -r examples/robot_kit rantaiclaw-robot-kit/
cp -r ~/.rantaiclaw rantaiclaw-robot-kit/dot-rantaiclaw

# Include models
mkdir -p rantaiclaw-robot-kit/models
cp ~/.rantaiclaw/models/ggml-base.bin rantaiclaw-robot-kit/models/
# Note: Ollama models are large, may want to download on target

# Create tarball
tar -czvf rantaiclaw-robot-kit.tar.gz rantaiclaw-robot-kit/

# Copy to USB
cp rantaiclaw-robot-kit.tar.gz /media/usb/TarBalls/
```
