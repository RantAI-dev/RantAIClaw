...
Complete guide to setting up a RantaiClaw-powered robot on Raspberry Pi 5.
...
mkdir -p ~/.rantaiclaw/models
cp models/ggml-base.bin ~/.rantaiclaw/models/
```
...
mkdir -p ~/.rantaiclaw/models/piper
cd ~/.rantaiclaw/models/piper
wget https://huggingface.co/rhasspy/piper-voices/resolve/main/en/en_US/lessac/medium/en_US-lessac-medium.onnx
wget https://huggingface.co/rhasspy/piper-voices/resolve/main/en/en_US/lessac/medium/en_US-lessac-medium.onnx.json

# Test
echo "Hello, I am your robot!" | piper --model ~/.rantaiclaw/models/piper/en_US-lessac-medium.onnx --output_file test.wav
aplay test.wav
```
...
# Build main rantaiclaw (optional, if using as agent)
cargo build --release
```
...
```bash
mkdir -p ~/.rantaiclaw
nano ~/.rantaiclaw/robot.toml
```

```toml
# ~/.rantaiclaw/robot.toml - Real Hardware Configuration
...
# Test speaker
echo "Testing speaker" | piper --model ~/.rantaiclaw/models/piper/en_US-lessac-medium.onnx --output_file - | aplay -D plughw:0,0
...
FIFO_PATH = "/tmp/rantaiclaw_sensors.fifo"
...
### Start RantaiClaw Agent

```bash
# Configure RantaiClaw to use robot tools
cat > ~/.rantaiclaw/config.toml << 'EOF'
...
cp ~/rantaiclaw/crates/robot-kit/SOUL.md ~/.rantaiclaw/workspace/

# Start agent
./target/release/rantaiclaw agent
```
...
# Start sensor loop
if [ ! -p /tmp/rantaiclaw_sensors.fifo ]; then
    mkfifo /tmp/rantaiclaw_sensors.fifo
fi
...
# Start rantaiclaw
cd ~/rantaiclaw
./target/release/rantaiclaw daemon &
AGENT_PID=$!
...
```bash
# /etc/systemd/system/rantaiclaw-robot.service
sudo tee /etc/systemd/system/rantaiclaw-robot.service << 'EOF'
[Unit]
Description=RantaiClaw Robot
After=network.target ollama.service

[Service]
Type=simple
User=pi
WorkingDirectory=/home/pi/rantaiclaw
...
sudo systemctl enable rantaiclaw-robot
sudo systemctl start rantaiclaw-robot

# Check status
sudo systemctl status rantaiclaw-robot
journalctl -u rantaiclaw-robot -f  # View logs
```
