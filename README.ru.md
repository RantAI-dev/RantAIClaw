...
<h1 align="center">RantaiClaw 🦀（Русский）</h1>
...
|---|---|---|---|
| 2026-02-19 | _Срочно_ | Мы **не аффилированы** с `openagen/rantaiclaw` и `rantaiclaw.org`. Домен `rantaiclaw.org` сейчас указывает на fork `openagen/rantaiclaw`, и этот домен/репозиторий выдают себя за наш официальный сайт и проект. | Не доверяйте информации, бинарникам, сборам средств и «официальным» объявлениям из этих источников. Используйте только этот репозиторий и наши верифицированные соцсети. |
| 2026-02-19 | _Важно_ | Официальный сайт пока **не запущен**, и мы уже видим попытки выдавать себя за RantaiClaw. Пожалуйста, не участвуйте в инвестициях, сборах средств или похожих активностях от имени RantaiClaw. | Ориентируйтесь только на этот репозиторий; также следите за [X (@rantaiclawlabs)](https://x.com/rantaiclawlabs?s=21), [Reddit (r/rantaiclawlabs)](https://www.reddit.com/r/rantaiclawlabs/), [Telegram (@rantaiclawlabs)](https://t.me/rantaiclawlabs), [Telegram CN (@rantaiclawlabs_cn)](https://t.me/rantaiclawlabs_cn), [Telegram RU (@rantaiclawlabs_ru)](https://t.me/rantaiclawlabs_ru) и [Xiaohongshu](https://www.xiaohongshu.com/user/profile/67cbfc43000000000d008307?xsec_token=AB73VnYnGNx5y36EtnnZfGmAmS-6Wzv8WMuGpfwfkg6Yc%3D&xsec_source=pc_search) для официальных обновлений. |
...
RantaiClaw — это производительная и расширяемая инфраструктура автономного AI-агента:
...
## Снимок бенчмарка (RantaiClaw vs OpenClaw, воспроизводимо)
...
> Примечание: результаты RantaiClaw получены на release-сборке с помощью `/usr/bin/time -l`. OpenClaw требует Node.js runtime; только этот runtime обычно добавляет около 390MB дополнительного потребления памяти. NanoBot требует Python runtime. PicoClaw и RantaiClaw — статические бинарники.

<p align="center">
  <img src="zero-claw.jpeg" alt="Сравнение RantaiClaw и OpenClaw" width="800" />
</p>
...
- `rantaiclaw --help`: ~`0.02s`, пик памяти ~`3.9MB`
- `rantaiclaw status`: ~`0.01s`, пик памяти ~`4.1MB`
...
RantaiClaw поддерживает нативные профили авторизации на основе подписки (мультиаккаунт, шифрование при хранении).

- Файл хранения: `~/.rantaiclaw/auth-profiles.json`
- Ключ шифрования: `~/.rantaiclaw/.secret_key`
...
<p align="center">
  <img src="docs/architecture.svg" alt="Архитектура RantaiClaw" width="900" />
</p>

| Подсистема | Trait | Встроенные реализации | Расширение |
|-----------|-------|---------------------|------------|
| **AI-модели** | `Provider` | Каталог через `rantaiclaw providers` (сейчас 28 встроенных + алиасы, плюс пользовательские endpoint) | `custom:https://your-api.com` (OpenAI-совместимый) или `anthropic-custom:https://your-api.com` |
...
При указании неподдерживаемого `runtime.kind` RantaiClaw завершается с явной ошибкой, а не молча откатывается к native.
